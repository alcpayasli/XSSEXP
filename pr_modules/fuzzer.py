#! /usr/bin/env python
# -*- coding: UTF-8 -*-
from pr_modules.brforce import *
...
class Fuzz ():
	...
	def fuzz(self):
		...
		vulns = []
		lines = [line.encode().decode('utf-8').strip() for line in payloadf.readlines()]
		...
		if ('login' in r.url):
			redir = input("Do you want to redirect to %s [Y/n]:" % r.url)
		...
		if (len(lines) != 0):
			...
			c = input('Do you want to print vulnerable links tested: [Y/n]:')
